...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TonicProst.swiftmodule/`
...
//      `import TonicProst` above would fail at compile time.
//
//   2. The static archive `libTonicProst.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TonicProst" }` and
//      `flattenPackage = "io.github.kotlinmania.tonicprost"` configuration in
...
//      via the `TonicProstLibrary` product.
//
...
final class TonicProstExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TonicProst swift module imported cleanly")
    }
}
