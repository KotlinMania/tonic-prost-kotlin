# Claude Code Project Instructions — tonic-prost-kotlin

## Project Overview

This is **tonic-prost-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tonic-prost`](https://crates.io/crates/tonic-prost).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tonic-prost/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tonic-prost>
